...
const locations = [
  { id: 0, name: "Café", position: { lat: 52.5349617, lng: 13.4182916}},
  { id: 1, name: "Zuhause", position: { lat: 52.52162837, lng: 13.4467040}},
  { id: 2, name: "Station", position: { lat: 52.5165879, lng: 13.4433214 }},
  { id: 3, name: "Arbeit", position: { lat: 52.5076, lng: 13.3902 }},
  { id: 4, name: "Persönlicher Ort", position: { lat: 52.5138820, lng: 13.3014155 }},
];
...
useEffect(() => {
  ...
  const smoothZoomAndPan = async (map) => {
    const initialZoom = map.getZoom();
    const intermediateZoom = initialZoom - 1; // Zoom ein bisschen raus
    const finalZoom = 15; // Endgültiges Ziel-Zoom-Level

    // Zoom-Out in kleinen Schritten
    await new Promise((resolve) => {
      map.setZoom(intermediateZoom);
      setTimeout(resolve, 500); // Warte 500ms, bevor weitergezoomt wird
    });

    // Verwende panTo, um die Karte sanft zur neuen Location zu bewegen
    const offsetX = 0.005; 
    const newCenter = {
      lat: selectedLocation.position.lat,
      lng: selectedLocation.position.lng + offsetX,
    };
    map.panTo(newCenter); // Sanftes Zentrieren der Karte

    // Zoom-In langsam auf das endgültige Level
    let currentZoom = intermediateZoom;
    const zoomInStep = () => {
      if (currentZoom < finalZoom) {
        currentZoom += 1;
        map.setZoom(currentZoom);
        setTimeout(zoomInStep, 300); // In kleinen Schritten alle 300ms weiter zoomen
      }
    };
    zoomInStep();
  };
...
{/* circle rendeering for last overview and eye-icon toggle */}
  ...
  {/* Render Circle only for the selected location and when hovered */}
    ...
    <div style={{ position: 'absolute', bottom: '325px', left: '20px', display: 'flex', flexDirection: 'column' }}>
          ...
          onMouseEnter={(e) => e.currentTarget.style.color = '#ff6347'} // Hover color change
