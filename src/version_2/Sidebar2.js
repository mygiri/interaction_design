...
const Sidebar2 = ({userID, defaultCenter, selectedLocation, preferences, visibleRadii, toggleRadiusVisibility, handleRadiusHover, handleLocationClick, handleNextLocation, handlePreviousLocation, fetchCurrentOptions, submitFeedbackData, radii, setnormalizedRadii, checkAndLockLocation, lockOnLocations, lockThroughSaveButton, unlockLocation, locations  }) => {
  ...
  const [progressByLocation, setProgressByLocation] = useState([0,0,0,0,0]);
  ...
  const handleProgress = (reset) => {
    setProgressByLocation((prev) => {
      const updatedProgress = [...prev];
      if (reset){
        updatedProgress[selectedLocation.id]=0
      }else{
        updatedProgress[selectedLocation.id] = Math.min(prev[selectedLocation.id] + 10, 100);
      }
        ...
        <div       
          ...
          onClick={async () => {
            if (!lockOnLocations[selectedLocation.id]) {
              ...
              await checkAndLockLocation(selectedLocation.id, locationRadius1, locationRadius2);
              handleProgress(0)
            }
          }}
          ...
          <button
            style={{
            position:'absolute',
            bottom: '5px',         // 5px from the bottom of the container
            right: '5px',
            width: '25px',
            height: '25px',
            borderRadius: '15%',
...
<div style={{ display: 'flex', justifyContent: 'center', gap: '5px' }}>
    {!lockOnLocations[selectedLocation.id] ? (
      <>
        <ProgressBar progress= {progressByLocation[selectedLocation.id]}/>
